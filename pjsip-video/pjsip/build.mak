export PJDIR := /Users/apple/Downloads/svn.local/pjsip
...
export CC_NAME := gcc
export TARGET_NAME := x86_64-apple-darwin17.2.0
...
# SDL flags
SDL_CFLAGS = -DPJMEDIA_VIDEO_DEV_HAS_SDL=1 -I/opt/local/include/SDL2 -D_THREAD_SAFE
SDL_LDFLAGS = -L/opt/local/lib -lSDL2

# FFMPEG flags
FFMPEG_CFLAGS =  -DPJMEDIA_HAS_LIBAVDEVICE=1 -DPJMEDIA_HAS_LIBAVFORMAT=1 -DPJMEDIA_HAS_LIBAVCODEC=1 -DPJMEDIA_HAS_LIBSWSCALE=1 -DPJMEDIA_HAS_LIBAVUTIL=1 -I/opt/local/include -DPJMEDIA_USE_OLD_FFMPEG=1 
FFMPEG_LDFLAGS =   -L/opt/local/lib -lavdevice -lavformat -lavcodec -lswscale -lavutil
...
export APP_LDLIBS := $(PJSUA_LIB_LDLIB) \
	...
	$(APP_THIRD_PARTY_EXT)\
	$(PJLIB_LDLIB) \
	-lopus -lssl -lcrypto -lm -lpthread  -framework CoreAudio -framework CoreServices -framework AudioUnit -framework AudioToolbox -framework Foundation -framework AppKit -framework AVFoundation -framework CoreGraphics -framework QuartzCore -framework CoreVideo -framework CoreMedia -framework VideoToolbox -L/opt/local/lib -lSDL2  -L/opt/local/lib -lavdevice -lavformat -lavcodec -lswscale -lavutil
